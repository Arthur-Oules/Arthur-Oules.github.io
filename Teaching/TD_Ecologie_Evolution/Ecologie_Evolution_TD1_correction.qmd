...
On peut donc appliquer des hypothèses de travail similaire et utiliser le même modèle mathématique. Ici, on cherchera à rtrouver les paramètres $\bar{z}(0)$, l'état ancestral et $\sigma^2$ le taux d'évolution.\
...
$$
\mathcal{L}(modèle(p) | \text{Tirage}) = P(\text{Tirage} | modèle(p)) = (p)^7 \cdot (1-p)^5
$$
...
Les chercheurs ont testé si le taux d''évolution des régimes alimentaires testés étaient différents en fonction du caractère pharyngognathe ou non-pharyngognathe.\
