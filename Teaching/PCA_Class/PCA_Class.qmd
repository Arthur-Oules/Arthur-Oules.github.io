...
author:
  ...
  - name: "Adapté de [Cosma Shalizi, Chapter 16: Principal Component Analysis](https://www.stat.cmu.edu/~cshalizi/uADA/16/lectures/17.pdf)"
description: "Lecture on the mathematical aspects of the PCA and its use in ecology."
image: figures/miniature.png
categories: Lecture
editor: source
freeze: auto
execute:
...
#| echo: false
#| label: fig-VarMSE
#| fig-cap: Equivalence entre Variance (à gauche) et MSE (à droite) sur l'exemple de notre jeu de données
#| fig-height: 7
...
variance_curve <- ggplot(data_res, aes(x = θ, y = sigma2)) +
  geom_smooth(color = "grey", se = FALSE) +
  geom_point(aes(frame = θ), colour = "red") +
  theme_minimal()
...
MSE_curve <- ggplot(data_res, aes(x = θ, y = MSE)) +
  geom_smooth(color = "grey", se = FALSE) +
  geom_point(aes(frame = θ), colour = "red") +
  theme_minimal()

subplot(
  ggplotly(variance), ggplotly(MSE),
  ggplotly(variance_curve), ggplotly(MSE_curve),
  nrows = 2, heights = c(.7, .3)
) |>
