...
angle <- data.frame(theta = round(seq.int(0, pi/2, length.out = 12)[-1], digits = 2))
...
variance <- ggplot(data = data_anim) +
  ...
  geom_text(
    aes(
      frame = θ,
      x = 1, y = -2.5,
      label = paste0("σ<sup>2</sup><sub>θ</sub> = ", as.character(round(variance, digits = 4)))
    )
  ) +
  coord_fixed(ratio = 1) +
  theme_minimal()
...
MSE <- ggplot(data = data_anim) +
  ...
  geom_text(
    aes(
      frame = θ,
      x = 1, y = -2.5,
      label = paste0("MSE(θ) = ", as.character(round(MSE, digits = 4)))
    )
  ) +
  coord_fixed(ratio = 1) +
  theme_minimal()

subplot(ggplotly(variance), ggplotly(MSE))  |>  animation_opts(frame = 500)
```
