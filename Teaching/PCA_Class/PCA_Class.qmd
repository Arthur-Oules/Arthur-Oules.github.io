...
Equivalence entre MSE et Variance sur l'exemple de notre jeu de données.
:::
...
output$angleplot <- renderPlot({
  ...
  p1 <- ggplot(data = data) +
    geom_segment(
      aes(
        x = scale_x, xend = (scale_x*cos(input$angle*pi/180) + scale_y*sin(input$angle*pi/180))*cos(input$angle*pi/180),
        y = scale_y, yend = (scale_x*cos(input$angle*pi/180) + scale_y*sin(input$angle*pi/180))*sin(input$angle*pi/180)
      ),
      linetype = 2,
      colour   = "grey"
    ) +
    geom_abline(
      aes(slope = tan(input$angle*pi/180), intercept = 0),
      colour = "grey"
      ) +
    geom_point(aes(x = scale_x, y = scale_y)) +
    geom_point(
      aes(
        x = (scale_x*cos(input$angle*pi/180) + scale_y*sin(input$angle*pi/180))*cos(input$angle*pi/180),
        y = (scale_x*cos(input$angle*pi/180) + scale_y*sin(input$angle*pi/180))*sin(input$angle*pi/180)
      ),
      colour = "red"
    ) +
    annotate(
      ...
      hjust = 0,
      size  = 5.5,
      label = paste("σ[θ]^2 ==", as.character(round(sd(data$scale_x*cos(input$angle*pi/180) + data$scale_y*sin(input$angle*pi/180)), digits = 4))),
      parse = TRUE
    ) +
    coord_fixed(ratio = 1, xlim = c(-3.5, 2.5), ylim = c(-3.5, 2.5)) +
    labs(
      x = "Trait 1 centré-réduit",
      y = "Trait 2 centré-réduit"
    ) +
  ...
  p2 <- ggplot(data = data) +
    ...
    annotate(
      ...
      label = paste(
        ...
        as.character(
          round(
            mean(data$scale_x**2 + data$scale_y**2) - mean((data$scale_x*cos(input$angle*pi/180) + data$scale_y*sin(input$angle*pi/180))**2),
            digits = 4
          )
        )
      )
