...
Considérons une matrice réelle $X \in \mathcal{M}_{n,p}(\mathbb{R})$ de taille $n \times p$ représentant $n$ individus et leurs $p$ paramètres associés :
...
On peut également considérer qu'il s'agit de $n$ réalisations de $p$ variables aléatoires $X_{1}, \dots, X_{p}$ telles que $\forall i \in [\![1,p]\!], X_{i} \sim \mathcal{N}(\mu, \sigma^{2})$.\
Ainsi, une ligne de $X$ correspond à un individu que l'on représente par le vecteur ligne $\vec{x_{i}} \in \mathbb{R}^{p}$ avec ses $p$ paramètres, qui sont une réalisation des $X_{i}$ :

$$
\vec{x_{i}} = \begin{bmatrix} x_{1} \dots x_{p}\end{bmatrix}_{i} \text{ et } \vec{x_{i}}^T = \begin{bmatrix}
      x_{1}\\
      \vdots \\
      x_{p}
...
On cherche à trouver un axe représenté par un vecteur $\vec{w} \in \mathbb{R}^{p}$ de l'espace des paramètres, donc à $p$ dimensions, tel qu'il maximise la variance du jeu de données, ou nuage de point, projetée sur celui-ci.
...
![Projection de $\vec{x_{i}}$ sur $\vec{w}$](figures/angle.png){#fig-proj}

Soit $\vec{x_{i}}$ un vecteur ligne de $X$ et $\vec{w} \in \mathbb{R}^{p}, \vec{w} \cdot \vec{w} = \|\vec{w}\| = 1$ un vecteur unitaire. On a :

$$
\text{cos}(\theta) = \frac{\vec{x_{i}} \cdot \vec{w}}{\|\vec{x_{i}}\|\|\vec{w}\|}
$$

Tel que la projection orthogonale de $\vec{x_{i}}$ sur $\vec{w}$ est de norme :

$$
\|\vec{x_{i}}\| \text{cos}(\theta) = \|\vec{x_{i}}\|  \frac{\vec{x_{i}} \cdot \vec{w}}{\|\vec{x_{i}}\|\|\vec{w}\|} = \vec{x_{i}} \cdot \vec{w}
$$
...
\| \vec{x_{i}} - (\vec{x_{i}} \cdot \vec{w})\vec{w} \|^2 &= (\vec{x_{i}} - (\vec{x_{i}} \cdot \vec{w})\vec{w}) \cdot  (\vec{x_{i}} - (\vec{x_{i}} \cdot \vec{w})\vec{w}) \\
                                 &= \|\vec{x_{i}}\|^2 - 2(\vec{w} \cdot \vec{x_{i}})^2 + (\vec{x_{i}} \cdot \vec{w})^2\vec{w} \cdot \vec{w} \\
                                 &= \vec{x_{i}} \cdot \vec{x_{i}} - (\vec{w} \cdot \vec{x_{i}})^2
...
\text{MSE}(\vec{w}) &= \frac{1}{n} \sum_{i = 1}^n(\|\vec{x_{i}}\|^2 - (\vec{w} \cdot \vec{x_{i}})^2) \\
                    &= \frac{1}{n} \sum_{i = 1}^n\|\vec{x_{i}}\|^2 - \frac{1}{n} \sum_{i = 1}^n(\vec{w} \cdot \vec{x_{i}})^2 \\

\end{align}

On veut trouver $\vec{w}$ tel que $\text{MSE}(\vec{w})$ soit minimale. Le premier terme est constant en fonction de $\vec{w}$, il reste donc à maximiser le second terme (il y a un $-$ !). On remarque que le second terme peut s'écrire $\frac{1}{n} \sum_{i = 1}^n(\vec{w} \cdot \vec{x_{i}})^2 = \mathbb{E}\left[(\vec{w} \cdot \vec{x_{i}})^2\right]$ or on a la relation $\text{Var}(X) = \mathbb{E}[X^2] - \mathbb{E}[X]^2$ d'où l'égalité :

\begin{align}

\frac{1}{n} \sum_{i = 1}^n(\vec{w} \cdot \vec{x_{i}})^2 &= (\frac{1}{n} \sum_{i = 1}^n \vec{w} \cdot \vec{x_{i}})^2 + \text{Var}(\vec{w} \cdot \vec{x_{i}}) \\
&= \text{Var}(\vec{w} \cdot \vec{x_{i}})

\end{align}

Car $\sum_{i = 1}^n \vec{w} \cdot \vec{x_{i}} = (\sum_{i = 1}^n\vec{x_{i}}) \cdot \vec{w} = 0$ puisque les $X_{p}$ sont des variables centrées de moyennes nulles donc $\frac{1}{n} \sum_{i = 1}^n \|\vec{x_{i}}\| = 0$.

Ainsi on a :

$$
\text{MSE}(\vec{w}) = \frac{1}{n}\sum_{i = 1}^n \|\vec{x_{i}}\|^2 - \text{Var}(\vec{w} \cdot \vec{x_{i}})
$$

Minimiser MSE revient donc à maximiser la variance des projections des $\vec{x_{i}}$ mais comment faire ?

En rappelant :

$$
X \vec{w}^{T} = \begin{bmatrix} x_{1,1} & \dots  & x_{1,p} \\
                                 \vdots & \ddots & \vdots \\
                                x_{n,1} & \dots  & x_{n,p}  \end{bmatrix} \times \begin{bmatrix} w_{1} \\ 
                                                                                                 \vdots \\
                                                                                                 w_{p} \end{bmatrix} = \begin{bmatrix} \sum_{i = 1}^{p} w_{i}.x_{i, 1}\\
                 \vdots \\
                 \sum_{i = 1}^{p} w_{i}.x_{i, n} \end{bmatrix} =
\begin{bmatrix} \vec{w} \cdot \vec{x_{1}} \\
                        \vdots \\
                \vec{w} \cdot \vec{x_{2}} \end{bmatrix}
$$
et

$$
(X\vec{w}^{T})^{T}(X\vec{w}^{T}) = \sum_{i = 1}^{n} (\vec{x_{i}} \cdot \vec{w})^{2}
$$
...
\sigma_{\vec{w}}^{2} = \text{Var}(\vec{w} \cdot \vec{x_{i}}) &= \frac{1}{n} \sum_{i = 1}^{n}(\vec{w} \cdot \vec{x_{i}})^{2} \\
                                                             &= \frac{1}{n}(X\vec{w}^{T})^{T}(X\vec{w}^{T}) \\
                                                             &=\frac{1}{n} \vec{w} X^{T} X \vec{w}^{T} \\
                                        \sigma_{\vec{w}}^{2} &= \vec{w} cov_{X^{T}, X} \vec{w}^{T}

\end{align}

avec $\vec{w}^{T}$ le vecteur colonne unitaire de taille $n \times 1$ et $cov_{X^T,X}$ la matrice de variance-covariance des $X_{p}$ variables aléatoires correspondants à nos paramètres :

\begin{align}

cov_{X^T, X} = \text{Cov}\left[X^T, X\right] &= \mathbb{E}\left[(X - \mu_{X})^T(X - \mu_{X})\right] \\
                                             &= \mathbb{E}\left[X^TX\right] \text{ en centrant les } X_{p}\\
\end{align}
