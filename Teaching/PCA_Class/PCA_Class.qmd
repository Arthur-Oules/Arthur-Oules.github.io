...
D'après le théorème de Pyhagore, on a :
...
Minimiser $\text{MSE}(\vec{w})$ revient donc à maximiser la variance des projections des $\vec{x_{i}}$ mais comment faire ?
...
avec $cov_{X^T,X}$ la matrice de variance-covariance des $X_{p}$ variables aléatoires de dimension $p \times p$ correspondants à nos paramètres :
...
\begin{align}
               ...
               \mathcal{L}(\vec{w}, \lambda) &\equiv \sigma_{\vec{w}}^2 - \lambda(\vec{w}^T\vec{w} - 1) \\
&\begin{cases}
...
Et ça nous arrange bien car la matrice de covariance a des propriétés intéressantes : c'est une matrice carrée (ici de dimensions $p \times p$), positive, symétrique et inversible. Donc il existe une décomposition en valeurs propres de cette matrice telle que $cov_{X^T,X}$ se factorise sous la forme :

$$
cov_{X^T,X} = X^TX = W \Lambda W^{-1}
$$

Avec $W$, de dimension $p \times p$, la matrice diagonale des vecteurs propres et $\Lambda$, de dimension $p \times p$, la matrice des valeurs propres de $cov_{X^T,X}$.\
...
On appelle $T$ la matrice des poids, la matrice des coordonnées du jeu de données dans le nouvel espace.
...
(fviz_eig(iris_pca, addlabels = TRUE) + coord_cartesian(ylim = c(0, 100))) | (fviz_pca_var(iris_pca, col.var = "black") + coord_cartesian(ratio = 1, xlim = c(-1.25, 1.25), ylim = c(-1.25, 1.25)))
```
